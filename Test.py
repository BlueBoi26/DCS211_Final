...
def fetch_census_data(state_fp):
    ...
    # Parse NAME field: "Autauga County, Alabama"
...
def fetch_bls_unemployment(state_fp, county_fips_list):
    ...
    # REDUCED BATCH SIZE: 20 is safer than 50 for large states like TX
    batch_size = 20
    results = []
    ...
    for i in range(0, len(all_series_ids), batch_size):
        ...
        try:
            ...
            if data['status'] == 'REQUEST_SUCCEEDED':
                ...
                # Print API messages if failed
                print(f"    - Batch warning: {data.get('message', 'Unknown error')}")
                
        except Exception as e:
            print(f"  [!] BLS API Batch Error: {e}")
        
        # SLOWER PACE: Sleep 1s between batches to avoid rate limits
        time.sleep(1)
...
def fetch_cdc_places(state_abbr):
    ...
    # Ensure we have FIPS (locationid)
    ...
    # Pivot on locationid (FIPS) instead of locationname
...
def main():
    ...
    for state_postal in states_to_process:
        ...
        # Merge 1: Base + FEMA (by Name)
        # (FEMA doesn't always provide clean FIPS in this endpoint, so name is best here)
        merged = pd.merge(df_base, df_fema, on="county_name", how="left")
        
        # Merge 2: + BLS (by FIPS)
        merged = pd.merge(merged, df_bls, on="fips_key", how="left")
        
        # Merge 3: + CDC (by FIPS now - much safer!)
        # Ensure FIPS types match (both strings)
        if not df_cdc.empty:
            df_cdc["fips_key"] = df_cdc["fips_key"].astype(str)
            merged = pd.merge(merged, df_cdc, on="fips_key", how="left")
        else:
            # Create empty health columns if CDC failed
    ...
    if final_dfs:
        ...
        # REMOVED BROADBAND FROM COLUMNS
        cols = [
